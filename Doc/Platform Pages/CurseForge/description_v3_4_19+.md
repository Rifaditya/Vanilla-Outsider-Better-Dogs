...
### 🏕️ Tamed Wolf Guard Mode *(Added in v3.4.19)*
Put your tamed wolves to work protecting your base while you're away:
- **Guard Post Activation**: Sneak + Right-click a tamed wolf with a bone (consumes 1 bone) to toggle Guard Mode and anchor it to its current block as a sentry post.
- **Personality Patrols**: Guards patrol in patterns that match their personality:
  - 💢 **Aggressive**: Wide outer perimeter sweeps, actively scanning for hostiles.
  - ✨ **Normal**: Sit-sentry at the post, or radial star patrols outward and back.
  - ❤️ **Pacifist**: Close protective orbital pacing around the post.
- **Auto-Target & Chase Caps**: Guards automatically attack hostiles within range (16–24 blocks) but are capped from chasing targets too far — they can't be lured away from your base.
...
- **Visual Indicators**: Subtle foot particles every 4 seconds (ASH / WHITE_ASH / MYCELIUM) show each guard's active mode at a glance.
- **Ownership Locked**: Only the wolf's owner can toggle Guard Mode or issue commands — other players are completely ignored.
...
> [!NOTE]
> **Server-Side Optional** *(since v3.6.0)*: This mod can run on the server only. Vanilla clients can connect to a modded server without installing the mod themselves.

> [!TIP]
> **Want a GUI?** Install the optional **[Cloth Config API](https://www.curseforge.com/minecraft/mc-mods/cloth-config)** and **[ModMenu](https://www.curseforge.com/minecraft/mc-mods/modmenu)** to get a full categorized settings screen inside the game — no commands needed! *(Added in v3.4.16)*

> [!TIP]
> **Too many options?** If the game rule screen feels cluttered, we highly recommend installing **[Collapsible Game Rules](https://www.curseforge.com/minecraft/mc-mods/collapsible-gamerules)**. It will automatically group the settings into clean, expandable folders!
