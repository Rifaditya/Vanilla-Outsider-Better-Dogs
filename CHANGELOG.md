...
## [1.0.72+1.21.11] - 2026-08-18 [YANKED - MISSING MOD ICON]
### Removed
...
## [1.0.68+1.21.11] - 2026-08-18
### Added
- 🐺 **Harmonic Chorus Howl Audio Polish (`WolfHowlHelper.java`)**:
  - Implemented randomized harmonic pitch variations ($0.85\text{F}$ to $1.20\text{F}$) for cascading pack chorus howls with native `SoundEvents.WOLF_HOWL`.

## [1.0.67+1.21.11] - 2026-08-17
### Fixed
...
## [1.0.65+1.21.11] - 2026-08-17
### Added
- 📐 **Visual Entity Scaling Synchronization (`WolfPersonalityStatHelper.java`, `WolfMixin.java`)**:
  - Bound genetic physical scale directly to vanilla `Attributes.SCALE`, seamlessly syncing hitbox and visual model rendering on 1.21.11.

## [1.0.64+1.21.11] - 2026-08-17
### Fixed
...
- 🌍 **Expanded Biome Spawning & Dynamic Climate Variants (`BetterDogsSpawning.java`, `WolfCoatVariantHelper.java`)**:
  ...
  - Integrated dynamic climate coat variant resolution (`WolfCoatVariantHelper`): Savanna packs spawn as Spotted, Badlands packs as Striped/Rusty, Plains/Meadow as Woods/Pale, with a 10% stray diversity roll.
  - Implemented dynamic climate-personality trait biasing and immediate spawn-time wild pack cluster synchronization, electing dominant Alpha leaders and linking pack follower UUIDs.
...
- 🤺 **Play Fighting & Social Sparring AI (`SmallFightGoal.java`)**:
  ...
  - Spawns happy villager particles, plays panting/playful shake sounds, and deals 0 damage.
...
- 🌸 **Puppy Curiosity AI (`BabyCuriosityGoal.java`)**:
  - Puppies explore natural vegetation, inspect interesting flora, and observe critters when idle.
- Added automated test suites `WolfGiftMeritsTest` and `BabyCuriosityTest`.

## [1.0.53+1.21.11] - 2026-08-16
### Added
...
## [1.0.32+1.21.11] - 2026-08-15
### Fixed
- 💥 **Startup Crash Fix**: Fixed `InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt` on Minecraft 1.21.11 by migrating entity serialization hooks from legacy `CompoundTag` to native `net.minecraft.world.level.storage.ValueOutput` (`addAdditionalSaveData`) and `ValueInput` (`readAdditionalSaveData`).
- Supersedes and replaces broken builds `1.0.28+1.21.11` through `1.0.31+1.21.11`.

## [1.0.31+1.21.11] - 2026-08-15 [BROKEN / CRASHED ON STARTUP]
> **CRITICAL FAILURE**: Crashed on game initialization with `InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt(Lnet/minecraft/class_2487;...)V! Expected (Lnet/minecraft/class_11372;...)V`. Minecraft 1.21.11 changed entity serialization parameters from `CompoundTag` to `ValueOutput` / `ValueInput`. Superseded by `[1.0.32+1.21.11]`.
### Added
...
## [1.0.30+1.21.11] - 2026-08-15 [BROKEN / CRASHED ON STARTUP]
> **CRITICAL FAILURE**: Crashed on game initialization with `InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt(CompoundTag)`. Superseded by `[1.0.32+1.21.11]`.
### Added
...
## [1.0.29+1.21.11] - 2026-08-15 [BROKEN / CRASHED ON STARTUP]
> **CRITICAL FAILURE**: Crashed on game initialization with `InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt(CompoundTag)`. Superseded by `[1.0.32+1.21.11]`.
### Added
...
## [1.0.28+1.21.11] - 2026-08-15 [BROKEN / CRASHED ON STARTUP]
> **CRITICAL FAILURE**: Crashed on game initialization with `InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt(CompoundTag)`. Reintroduced legacy `CompoundTag` parameter descriptors into `WolfMixin.java`. Superseded by `[1.0.32+1.21.11]`.
### Added
...
## [1.0.20+1.21.11] - 2026-08-14
### Fixed
- 💥 **Startup Crash Fix**: Fixed `InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt` on Minecraft 1.21.11 by removing obsolete `addAdditionalSaveData(CompoundTag)` injections superseded by the 1.21.11 serialization system.

## [1.0.19+1.21.11] - 2026-08-14 [BROKEN / CRASHED ON STARTUP]
> **CRITICAL FAILURE**: Crashed on game initialization with `org.spongepowered.asm.mixin.injection.throwables.InvalidInjectionException: Invalid descriptor on WolfMixin->@Inject::betterdogs$writeNbt(CompoundTag)`. Minecraft 1.21.11 changed entity serialization parameters from `CompoundTag` to `ValueOutput` (`class_11372`). Superseded by `[1.0.20+1.21.11]`.
### Changed
...
## [1.0.18+1.21.11] - 2026-08-14
### Added
- 🏷️ **Dedicated 'Better Dogs' GameRules Category**:
  - Registered custom `BETTER_DOGS` category so all 80+ mod GameRules appear in their own section in the in-game world creation and edit gamerules menu.
  - Aligned all `en_us.json` GameRule titles and tooltip description keys (`gamerule.bd_*`).

## [1.0.17+1.21.11] - 2026-08-14
### Added
...
## [1.0.16+1.21.11] - 2026-08-14
### Fixed
- 💥 **Startup Crash Fix**: Fixed `NoClassDefFoundError: net/minecraft/class_1928$class_5198` (`GameRules.Category`) on Minecraft 1.21.11 by configuring native `minecraft_version=1.21.11` dependencies and registering rules with the 1.21.11 `GameRuleCategory` API.

## [1.0.15+1.21.11] - 2026-08-14 [BROKEN / CRASHED ON STARTUP]
> **CRITICAL FAILURE**: Crashed on game initialization with `java.lang.NoClassDefFoundError: net/minecraft/class_1928$class_5198` (`GameRules.Category`). Subproject was compiled against 1.21.1 mappings instead of 1.21.11 where `GameRules.Category` was replaced by `GameRuleCategory`. Superseded by `[1.0.16+1.21.11]`.
### Added
...
## [1.0.14+1.21.11] - 2026-08-14
### Changed
- **NBT & DNA Determinism Alignment**: Harmonized `WolfPersistentData` constants and deterministic DNA seed salt (`^ 5829103L`).
- **Exact Minecraft Target Constraint**: Updated `fabric.mod.json` `minecraft` dependency bound to target exact version `"minecraft": "1.21.11"`.

## [1.0.13+1.21.11] - 2026-08-14
### Added
