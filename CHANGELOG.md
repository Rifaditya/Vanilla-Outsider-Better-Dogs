...
- 🌐 **French (`fr_fr`, `fr_ca`) Localization & Player Guide**:
  - Added complete `assets/vanilla-outsider-better-dogs/lang/fr_fr.json` and `fr_ca.json` matching all 428 translation keys.
...
### Fixed
- **MC 26.3 NaturalSpawner Mixin Fix**: Resolved world tick crash (`InvalidInjectionException`) by updating `NaturalSpawnerMixin.mobsAt` descriptor to match MC 26.3's refactored `NaturalSpawner.mobsAt` method signature.
- **GameRule Registry Fix**: Fixed game initialization crash (`IllegalStateException: Adding duplicate key`) caused by redundant `betterdogs:bd_creeper_awareness` registration.
