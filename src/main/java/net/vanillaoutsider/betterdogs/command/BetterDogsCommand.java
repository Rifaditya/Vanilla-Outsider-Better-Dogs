...
/**
 * Dedicated single-purpose Brigadier command tree builder and executor for /betterdogs and /bd (1.21.11).
 */
public class BetterDogsCommand {
    ...
    private static int executeStatus(CommandSourceStack source) {
        ...
        sb.append(" §7Aggro: §fHP ").append(rules.get(BetterDogsGameRules.BD_AGGRO_HEALTH))
                .append(", SPD +").append(rules.get(BetterDogsGameRules.BD_AGGRO_SPEED_PCT))
                .append("%, DMG +").append(rules.get(BetterDogsGameRules.BD_AGGRO_DMG_PCT)).append("%\n");
        sb.append(" §7Pacifist: §fHP +").append(rules.get(BetterDogsGameRules.BD_PACI_HEALTH))
                .append(", SPD ").append(rules.get(BetterDogsGameRules.BD_PACI_SPEED_PCT))
                .append("%, DMG ").append(rules.get(BetterDogsGameRules.BD_PACI_DMG_PCT)).append("%\n");

        sb.append("§e[Environmental & Safety]§r\n");
        sb.append(" §7Storm Anxiety: ").append(formatBool(rules.get(BetterDogsGameRules.BD_STORM_ANXIETY)))
                .append(" §7| Cliff Safety: ").append(formatBool(rules.get(BetterDogsGameRules.BD_CLIFF_SAFETY)))
                .append(" §7| Creeper Evasion: ").append(formatBool(rules.get(BetterDogsGameRules.BD_CREEPER_EVASION_ENABLED))).append("\n");
        sb.append(" §7Friendly Fire Protection: ").append(formatBool(rules.get(BetterDogsGameRules.BD_FRIENDLY_FIRE)))
                .append(" §7| Actionbar Subtitles: ").append(formatBool(rules.get(BetterDogsGameRules.BD_ACTIONBAR_FEEDBACK))).append("\n");

        sb.append("§e[Tactics & Utilities]§r\n");
        sb.append(" §7Goat Horn Range: §a").append(rules.get(BetterDogsGameRules.BD_HORN_COMMAND_RANGE)).append(" blocks§r")
                .append(" §7| Flanking Tactics: ").append(formatBool(rules.get(BetterDogsGameRules.BD_PACK_FLANKING_TACTICS)))
                .append(" §7| Nemesis Memory: ").append(formatBool(rules.get(BetterDogsGameRules.BD_NEMESIS_SYSTEM))).append("\n");

        sb.append("§e[Genetics & Breeding]§r\n");
        sb.append(" §7Max Litter Size: §a").append(rules.get(BetterDogsGameRules.BD_WOLF_LITTER_MAX_SIZE))
                .append(" §7| Scale Range: §a").append(rules.get(BetterDogsGameRules.BD_WOLF_MIN_SCALE_PERCENT)).append("%-")
                .append(rules.get(BetterDogsGameRules.BD_WOLF_MAX_SCALE_PERCENT)).append("%§r")
                .append(" §7| Autonomous Feeding: ").append(formatBool(rules.get(BetterDogsGameRules.BD_DOGS_EAT_RAW_FOOD)));
    ...
    private static String formatBool(Boolean val) {
        return (val != null && val) ? "§aEnabled§r" : "§cDisabled§r";
    }
    ...
    private static int executeGet(CommandSourceStack source, String ruleArg) {
        ...
        if (BOOL_MAP.containsKey(normalized)) {
            GameRule<Boolean> rule = BOOL_MAP.get(normalized);
            boolean val = server.overworld().getGameRules().get(rule);
            ...
            GameRule<Integer> rule = INT_MAP.get(normalized);
            int val = server.overworld().getGameRules().get(rule);
    ...
    private static int executeSet(CommandSourceStack source, String ruleArg, String valueArg) {
        ...
        if (BOOL_MAP.containsKey(normalized)) {
            ...
            server.overworld().getGameRules().set(rule, parsed, server);
    ...
    private static int executeReset(CommandSourceStack source) {
        ...
        for (Map.Entry<String, GameRule<Boolean>> entry : BOOL_MAP.entrySet()) {
            ...
            rules.set(entry.getValue(), def, server);
        }
