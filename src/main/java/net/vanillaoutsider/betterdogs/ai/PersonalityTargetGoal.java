...
public class PersonalityTargetGoal {
    ...
    public static boolean shouldTargetEntity(Wolf wolf, LivingEntity target, boolean isOwnerAttackingTarget) {
        ...
        if (wolf instanceof WolfExtensions ext) {
            ...
            switch (personality) {
                case PACIFIST -> {
                    // Pacifists NEVER initiate attacks or join owner offensive attacks
                    if (isOwnerAttackingTarget) {
                        return false;
                    }
                    // Pacifists ONLY defend IF target is actively attacking the wolf or its owner
                ...
                case AGGRESSIVE -> {
                    // Aggressive dogs actively scan and attack hostiles within 20 blocks
