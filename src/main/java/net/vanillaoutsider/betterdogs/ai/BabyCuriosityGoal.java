...
public class BabyCuriosityGoal extends Goal {
    ...
    private boolean findSomethingInteresting() {
        if (this.wolf.getRandom().nextFloat() < 0.6f) {
            ...
            List<LivingEntity> nearby = this.wolf.getCommandSenderWorld().getEntitiesOfClass(LivingEntity.class, this.wolf.getBoundingBox().inflate(10.0), predicate);
            if (!nearby.isEmpty()) {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            BlockState state = this.wolf.getCommandSenderWorld().getBlockState(pos);
    ...
    public static boolean isInterestingBlock(BlockState state) {
        ...
        return state.is(Blocks.TALL_GRASS) || state.is(Blocks.GRASS)
