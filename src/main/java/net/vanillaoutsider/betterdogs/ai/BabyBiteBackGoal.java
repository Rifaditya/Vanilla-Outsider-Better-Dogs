    ...
    public void tick() {
        ...
        if (distSq <= reachSq) {
            ...
            if (level != null && !level.isClientSide()) {
                ...
                level.playSound(null, this.wolf.getX(), this.wolf.getY(), this.wolf.getZ(), SoundEvents.WOLF_SHAKE, SoundSource.NEUTRAL, 0.8F, 1.4F);

                DamageSource source = level.damageSources().mobAttack(this.wolf);
                if (!WolfFriendlyFireHelper.shouldCancelDamage(this.wolf, source)) {
                    this.target.hurtServer((net.minecraft.server.level.ServerLevel) level, source, 1.0F);
                }
            }
